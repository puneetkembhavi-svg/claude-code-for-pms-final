...
### Module 5 — brief and prototype delivered to Helen
Helen's actual ask (`05-super-speed/director-request.txt`): explicitly does **not** want a quiet revert of the timeout — "I don't want to just quietly change a number and call it handled." Wants something designed from the responder's point of view, not a setting; something a handler like Kip would notice; a one-pager, ideally something clickable, not polished.

Delivered `05-super-speed/brief.md` — direction: **"Same speed. Fairer memory. No permanent lockouts."** Keep the 60s window; stop scoring a missed offer the same as a refusal; let quiet time itself heal a responder's standing instead of requiring lucky timing. Five decisions resolved on the record: (1) no new UI needed for v1 — Kip notices because pings resume, not a label; (2) recovery targeted at 2–3 weeks; (3) currently-stuck responders (Vesper, Meteor Mite, etc.) get a one-time correction, not just the new ongoing mechanism; (4) the same mechanism also eases back over-served responders (The Gale) — not extra scope; (5) detection stops depending on tickets — an automatic low-ping-threshold flag, since the worst-hit responders never complained.

Delivered `05-super-speed/prototype.html` — a clickable before/after demo, single file, real CSV data. Shows 4 responders across 3 handlers (Meteor Mite & The Gale under Kip, Vesper under Aunt Dot, Captain Vantage under Ambrose) so it's not just Kip's anecdote. Toggle "Today's system" vs "Proposed fix" + fast-forward weeks: under today's system quiet responders stay frozen forever (proves reverting the timeout alone wouldn't help them); under the fix all four converge to a normal pace within ~3 simulated weeks. Includes a live "gap between busiest and quietest responder" headline stat, a real-vs-projected divider on the charts, and an end-of-run summary table — built after Helen-readiness passes to make it self-explanatory without the PM narrating it live.
